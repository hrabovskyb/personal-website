...
function slideShow() {
  ...
  var globals = {
    ...
    startButtonID: "slideShowButton", // The ID of the <button> element that toggles the slide show on and off.
    backButtonID: "slideShowBack",
    forwardButtonID: "slideShowForward",
    buttonStartText: "Media/play_round_small.png", // Text used in the slide show toggle button.
    buttonStopText: "Media/pause_round_small.png", // Text used in the slide show toggle button.    
    ...
    startButtonObject: null, // If present, will contain a reference to the <button> element that toggles the slide show on and off. The initial assumption is that there is no such button element (hence the false value).
    backButtonObject: true,
    forwardButtonObject: true,
    ...
    slideShowRunning: null, // Used to record when the slide show is running and when it's not. The slide show is always initially running.    
  ...
  if (globals.startButtonObject) {
    globals.startButtonObject.addEventListener('click', toggleSlideShow, false); // This callback is used to toggle the slide show on and off.
    globals.backButtonObject.addEventListener('click', slideBack, false);
    globals.forwardButtonObject.addEventListener('click', slideForward, false);
  } 
  ...
  function initializeGlobals() {   
    ...
    globals.startButtonObject = (document.getElementById(globals.startButtonID) ? document.getElementById(globals.startButtonID) : null); 
    globals.backButtonObject = (document.getElementById(globals.backButtonID) ? document.getElementById(globals.backButtonID) : null);  
    globals.forwardButtonObject = (document.getElementById(globals.forwardButtonID) ? document.getElementById(globals.forwardButtonID) : null);     
  ...
  function insufficientSlideShowMarkup() {
    if (!globals.wrapperObject) { // There is no wrapper element whose ID is globals.wrapperID - fatal error.
      if (globals.startButtonObject) {
        globals.startButtonObject.style.display = "none"; // Hide the not needed slide show button element when present.
      }
      return true;
    }
    ...
    if (!globals.slideImages.length) { // There needs to be at least one slide <img> element - fatal error.
      ...
      if (globals.startButtonObject) {
        globals.startButtonObject.style.display = "none"; // Hide the not needed slide show button element.
      }
  ...
  function initializeSlideShowMarkup() {  
    ...
    if (globals.startButtonObject) {
      globals.startButtonObject.src = globals.buttonStartText;
    }
  } // initializeSlideShowMarkup
...
function slideBack() {
  	...
  	if (globals.slideIndex <= 0) {
    	globals.slideIndex = globals.slideImages.length;
    }
  ...
  function slideForward(){
  	var crurentSlide = globals.slideImages[globals.slideIndex];
  	++(globals.slideIndex);
  ...
  function toggleSlideShow() {
    if (globals.slideShowRunning) {
      ...
      startSlideShow();
      if (globals.startButtonObject) { 
        globals.startButtonObject.src = globals.buttonStopText; 
      }            
    }
